...
#### Scenario: Insufficient balance takes precedence over the unallocated-balance cap
...
- **THEN** the system rejects the disbursement with `INSUFFICIENT_BALANCE` and never evaluates the unallocated-balance cap

### Requirement: Loan Disbursement Cannot Exceed the Source Account's Unallocated Balance
The system SHALL reject a loan disbursement whose `amount` exceeds the source account's unallocated balance (computed balance minus the sum of that account's `GoalAllocationEvent` entries), with no consent or override path, checked only after the account's insufficient-balance check has already passed.
