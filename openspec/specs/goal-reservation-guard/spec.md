# goal-reservation-guard Specification

## Purpose

Protect funds a user has earmarked toward goals from being silently spent by non-goal transactions. When a new expense, investment contribution, or outgoing transfer would dip into an account's goal-reserved (unallocated-vs-allocated) balance, the system detects the shortfall, walks the user through an explicit consent flow to choose which goal(s) absorb it and whether the dip is permanent or expected to be returned, and tracks any such promise to return the funds through to resolution.
